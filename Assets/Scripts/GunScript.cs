...
public void Fire(float fire, int i)
  {
	...
	if (cooldown <= 0)
	{
      ...
      SoundManager.instance.bangBang(GunSound, soundPitch); // plays designated gun sound
	    ...
	    float r = Random.Range (-50, 50);  // random float to make the spray deviate a bit.
	    recoilVec = Main.GetUnitVector2( (gunAngle+90+ (r/50)*(100/accuracy)) * Mathf.Deg2Rad )*-1; // (i/50) is a float between -1 and 1 - for a random distribution
